...
if __name__ == "__main__":
    ...
    hostnametx = argstx.hostname
    porttx = argstx.port
    addresstx = argstx.address
    if not (2 < len(addresstx) < 6):
        print(f'Invalid address {addresstx}. Addresses must be 3 to 5 ASCII characters.')
        sys.exit(1)
    # Connect to pigpiod
    print(f'Connecting to GPIO daemon on {hostnametx}:{porttx} ...')
    pitx = pigpio.pi(hostnametx, porttx)
    if not pitx.connected:
        print("Not connected to Raspberry Pi ... goodbye.")
        sys.exit()
    # Create NRF24 object.
    # PLEASE NOTE: PA level is set to MIN, because test sender/receivers are often close to each other, and then MIN works better.
    nrftx = NRF24(pitx, ce=7, payload_size=RF24_PAYLOAD.DYNAMIC, channel=50, data_rate=RF24_DATA_RATE.RATE_2MBPS,
                  pa_level=RF24_PA.LOW)
    nrftx.set_address_bytes(len(addresstx))
    nrftx.open_writing_pipe(addresstx)
    # Display the content of NRF24L01 device registers.
    nrftx.show_registers()
    ...
    GPIO.output(26, 1)
    try:
        print(f'Send to {addresstx}')
        count = 0
        while True:
            # As long as data is ready for processing, process it.
            while nrftx.data_ready():
                #-------------------------------------------------------------------------------------------------------
                # Capture frame-by-frame
                try:
                    ret, frame = caplist[capnum-1].read()
                ...
                payloadtx = struct.pack("<Bf", 0x01,frame)
                # Send the payload to the address specified above.
                nrftx.reset_packages_lost()
                nrftx.send(payloadtx)
                try:
                    nrftx.wait_until_sent()
                except TimeoutError:
                    print('Timeout waiting for transmission to complete.')
                    # Wait 10 seconds before sending the next reading.
                    time.sleep(0.1)
                    continue
                if nrftx.get_packages_lost() == 0:
                    print(f"Success: lost={nrftx.get_packages_lost()}, retries={nrftx.get_retries()}")
                else:
                    print(f"Error: lost={nrftx.get_packages_lost()}, retries={nrftx.get_retries()}")
                # Sleep 100 ms.
                time.sleep(0.1)
    except:
        traceback.print_exc()
